...
SELECT calisan_id AS id FROM workers;--görüntülenen raporda calisan_id sütunu id olarak geçici isimlendirildi.
...
--calisanlar3 tablosunda ikinci en yüksek maaşı alan çalışanın bilgilerini gösteriniz
...
SELECT marka_id,marka_isim,(SELECT COUNT(DISTINCT(sehir)) 
							FROM calisanlar3 
							WHERE isyeri=marka_isim) AS sehir_sayisi--correlated subquery
FROM markalar;

--NOT:SELECT DISTINCT(sehir) FROM calisanlar3;
